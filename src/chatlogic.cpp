...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        auto newNode = std::find_if(_nodes.cbegin(), _nodes.cend(),
                                [&id](const std::unique_ptr<GraphNode>& node) { return node->GetID
                                () ==
                                id; });

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.cend())
                        {
                            _nodes.emplace_back( std::make_unique<GraphNode>(id, this));
                            newNode = _nodes.cend() - 1; // get iterator to last element
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto parentNode = std::find_if(_nodes.begin(), _nodes.end(),
                                    [&parentToken](const auto& node) { return
                                    node->GetID()
                                    == std::stoi(parentToken->second); });
                            auto childNode = std::find_if(_nodes.begin(), _nodes.end(),
                                    [&childToken](const auto& node) { return node->GetID() ==
    ...
    for (auto it = std::cbegin(_nodes); it != std::cend(_nodes); ++it)
    {
    ...
    ChatBot cb("../images/chatbot.png"); // Create a local Chatbox instance on the stack
